...
| Secrets and key management | Security/Platform | 本地 runtime 的 macOS Keychain 保持 local-only；deployment 目标合同固定为 platform-managed version ID + workload identity → authenticated private regular-file projection → read-only mount → `MountedFileSecretProvider`。adapter selection 为 `blocked_platform_selection`，未选择或实现 vendor | 当前仍缺平台选择、真实 workload identity/secret versions、稳定 preflight/轮换/60 分钟回滚演练与 separate approvals。Production Go remains false；仓库、环境变量、argv、镜像、日志、Frappe site config、审计值/哈希均不得承载明文 secret | [deployment lifecycle](deployment-secrets.md), [projection template](../infra/prod/secret-provider-v1.template.json), [threat model](governance/threat-model.md) |
