"""Fail-closed bench helper for the Email Gateway authority service identity."""
...
_USER = "email-gateway-authority@localhost.invalid"
_ROLE = EMAIL_GATEWAY_AUTHORITY_ROLE
_AUTH_REF = "email-gateway-authority-v1"
_DEFAULT_SITE = "gbos.localhost"
_PURPOSES = ["email_gateway_authority"]
...
class EmailGatewayAuthorityProvisioningError(RuntimeError):
    """A non-sensitive provisioning refusal."""


def provision_email_gateway_authority(confirm_local_pilot: Any = False) -> dict[str, str]:
    try:
        receipt = _provision(confirm_local_pilot)
        frappe.db.commit()
        return receipt
    except EmailGatewayAuthorityProvisioningError:
        frappe.db.rollback()
        raise
    except Exception:
        frappe.db.rollback()
        raise EmailGatewayAuthorityProvisioningError("authority provisioning failed") from None


def _provision(confirm_local_pilot: Any) -> dict[str, str]:
    if confirm_local_pilot is not True:
        raise EmailGatewayAuthorityProvisioningError("local-pilot confirmation required")
    if os.environ.get("GBOS_PRODUCTION_ENABLED", "").strip().casefold() in _PRODUCTION_VALUES:
        raise EmailGatewayAuthorityProvisioningError("production environment is not allowed")
    if any(
        key in os.environ
        for key in (
            "GBOS_EMAIL_GATEWAY_AUTHORITY_API_KEY",
            "GBOS_EMAIL_GATEWAY_AUTHORITY_API_SECRET",
        )
    ) or any(
        ...
        raise EmailGatewayAuthorityProvisioningError("legacy credential environment is not allowed")
    site_id = _active_site()
    _validate_config(site_id)
    _validate_role()
    api_key = _credential(
        "GBOS_EMAIL_GATEWAY_AUTHORITY_API_KEY_FILE",
        "gbos_email_gateway_authority_api_key_file",
    )
    api_secret = _credential(
        "GBOS_EMAIL_GATEWAY_AUTHORITY_API_SECRET_FILE",
        "gbos_email_gateway_authority_api_secret_file",
    )
    if frappe.db.exists("User", _USER):
        user = frappe.get_doc("User", _USER)
        if not _is_exact_user(user, api_key, api_secret):
            raise EmailGatewayAuthorityProvisioningError("existing identity drift")
        return _receipt("skipped", site_id)
    user = frappe.get_doc(
        {
            "doctype": "User",
            "email": _USER,
            "first_name": "Email Gateway Authority Service",
            "enabled": 1,
    ...
    if not _is_exact_user(persisted, api_key, api_secret):
        raise EmailGatewayAuthorityProvisioningError("created identity failed verification")
...
def _active_site() -> str:
    ...
    if _SITE.fullmatch(site) is None or (closed is not None and closed != site):
        raise EmailGatewayAuthorityProvisioningError("active site is not allowed")
...
def _validate_config(site: str) -> None:
    ...
    if (
        ...
        raise EmailGatewayAuthorityProvisioningError("identity configuration is invalid")
...
def _validate_role() -> None:
    if (
        ...
        raise EmailGatewayAuthorityProvisioningError("service role is invalid")
...
def _credential(environment_key: str, config_key: str) -> str:
    ...
    if environment_path is not None and config_path is not None:
        raise EmailGatewayAuthorityProvisioningError("credential file is invalid")
    ...
    if (
        ...
        or not _SAFE_FILENAME.fullmatch(path.name)
    ):
        raise EmailGatewayAuthorityProvisioningError("credential file is invalid")
    try:
        before = path.lstat()
        ...
        raise EmailGatewayAuthorityProvisioningError("credential file is invalid") from None
    try:
...
def _receipt(status: str, site: str) -> dict[str, str]:
    return {"status": status, "user": _USER, "role": _ROLE, "auth_ref": _AUTH_REF, "site_id": site}


__all__ = ["EmailGatewayAuthorityProvisioningError", "provision_email_gateway_authority"]
